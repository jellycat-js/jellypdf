...
describe('PlaywrightHandler', () => {
	...
	const mockBrowser = {
		newContext: jest.fn().mockResolvedValue(mockContext as unknown as BrowserContext),
		close: jest.fn()
	}
	...
	beforeEach(() => {
		...
		jest.spyOn(chromium, 'launch').mockResolvedValue(mockBrowser as unknown as Browser)
	})
	...
	describe('renderPdf', () => {
		...
		it('should generate a PDF after navigation', async () => {
			...
			expect(mockPage.goto).toHaveBeenCalledWith(input, { waitUntil: 'networkidle' })
